...
def search_repos_in_dates(q, from_date, to_date, last_update_date, firstCall=False):
    query = f'{q} created:{from_date}..{to_date} pushed:>{last_update_date}'
    ...
    if count < 1000 or from_date == to_date:
        ...
        return [{"from_date": from_date, "to_date": to_date, "count": count, "results": repositories}]
    else:
        middle = from_date + (to_date - from_date)/2
        return search_repos_in_dates(q, from_date, middle, last_update_date) +\
               search_repos_in_dates(q, middle + timedelta(days=1), to_date, last_update_date)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'search-repos':
        ...
        try:        
            ...
            last_update_date = datetime.strptime(config['DEFAULT']['last_update_date'], "%Y/%m/%d").date()
        except:
        ...
        if query_detailed_search:
            search_results = search_repos_in_dates(query_detailed_search, from_date, to_date, last_update_date, firstCall=True)
            cnt = 0
            maven_projects = []
            maven_repos_with_recent_pr = []
            for search_result in search_results:
                filtered_repos = search_result['results']
                for repo in filtered_repos:
                    ...
                    if is_maven:
                        maven_projects.append(repo)

            for repo in maven_projects:
                pull_requests = repo.get_pulls(state="closed", sort="updated", direction="desc")

                if pull_requests.totalCount < 1:
                    continue
                last_pr = pull_requests[0]
                if last_pr.closed_at.date() > datetime.strptime("2019/11/09", "%Y/%m/%d").date():
                    maven_repos_with_recent_pr.append(repo)

            f = open('selected_repos.txt', 'w')
            for ele in maven_repos_with_recent_pr:
                f.write(ele.git_url + ',' + str(ele.stargazers_count) + '\n')
